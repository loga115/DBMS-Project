...
class ParkingSystem:
    ...
    def open_remove_vehicle_window(self):
        ...
        # TODO: Add code for remove vehicle window
    ...
    def open_view_car_window(self):
        ...
        self.entry_parking_id = tk.Entry(self.output_frame, width=30)
        self.entry_parking_id.grid(row=0, column=1, padx=(0, 20), pady=(0, 10))
        ''' 3      p_LicensePlate OUT Vehicle.LicensePlate%TYPE,
  4      p_VehicleName OUT Vehicle.VehicleName%TYPE,
  5      p_VehicleCompany OUT Vehicle.VehicleCompany%TYPE,
  6      p_VehicleType OUT Vehicle.VehicleType%TYPE,
  7      p_VehicleDesc OUT Vehicle.VehicleDesc%TYPE'''
    ...
    def get_fee(self):
        if self.entry_parking_id.get() is not None:
            parking_id = self.entry_parking_id.get()
        if self.entry_license_plate.get() is not None:
            license_plate = self.entry_license_plate.get()
        else:
        ...
        try:
            ...
            self.entry_license_plate.insert(0, license_plate if license_plate is not None else "")
            self.entry_license_plate.config(state="readonly")
